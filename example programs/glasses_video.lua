...
local function write_pixels(at, qdata)
    while at <= #qdata.results do
        ...
        monitor.setCursorPos(xy_pixels[at][1], xy_pixels[at][2])
...
local function yield() if os.epoch("utc") - yield_point >= 1000 then os.queueEvent("yield") os.pullEvent("yield") yield_point = os.epoch("utc") end end

local function write_data()
    dt.queueRaycasts(max_distance, pixels, euler_mode, do_cache)
    local at = 1
    local qdata = dt.getQueuedData()
